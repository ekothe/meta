...
data_cleaned<-data_cleaned %>% arrange(.dots=c(data_cleaned$Study.ID))

## Drop unnecessary columns from working data file
data_reduced<-data_cleaned[,c("Timestamp","Study.ID", "Intervention.Mean", "Intervention.SD", "Intervention.N", "Control.Mean", "Control.SD", "Control.N")]

## Identify duplicate cases by removing exact mataching and keeping duplicate entries with inconsistent data
data_exact<-data_reduced %>% distinct(Study.ID, Intervention.Mean, Intervention.SD, Intervention.N, Control.Mean, Control.SD, Control.N, .keep_all = TRUE)
data_exact<-data_exact %>% mutate(row.ID = row_number())

data_fuzzy<-data_reduced %>% distinct(Study.ID, .keep_all = TRUE)
```
